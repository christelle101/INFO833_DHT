...
public class HWTransport implements Protocol {
    ...
    public void send(Node src, NodeDHT dest, Object msg, int pid) {
	long delay = getLatency(src,dest);
    Node dest2 = (Node) dest; //
	EDSimulator.add(delay, msg, dest2, pid);
    }
    
    
    //latence random entre la borne min et la borne max
    public long getLatency(Node src, NodeDHT dest) {
